...
def initial_choice(max_value):
    '''
    Página inicial do quiz, mostra quais capítulos se pode fazer o quiz
    é necessário passar o 'max_value' para podermos identificar quantas questões
    ...
    # Uso de st.form para evitar duplo envio (loop)
        ...
        if submit_banco:
            if novo_banco.strip():
                ...
                if match_id and match_gid:
                    ...
                    # Verifica nos secrets (adicionado check de segurança)
                    if "SHEET_ID" in st.secrets and "SHEET_GID" in st.secrets:
    ...
    # Lista de Capítulos escolhidos pelo usuário
    escolhas_cap = st.pills(label='Qual capítulo deseja simular a prova', 
    ...
    # Botão iniciar só é ativado quando o usuário escolher pelo menos 1 capítulo
    iniciar = st.button('Iniciar', disabled=(len(escolhas_cap) == 0))
    
    if iniciar:
        return escolhas_cap, num_questoes
    
    return None # Retorno padrão caso não inicie